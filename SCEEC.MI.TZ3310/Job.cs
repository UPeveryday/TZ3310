...
namespace SCEEC.MI.TZ3310
{
    ...
    public struct WindingType : IComparable<WindingType>
    {
        ...
        public Parameter.ZldzStation TozldzStation(JobList job)
        {
            if (job.DCResistance.ZcEnable && (this.windingType == 2))
            {
                return Parameter.ZldzStation.注磁抵押;
            }
            else
            {
                switch (this.windingType)
    ...
    public struct JobParameter
    {
        ...
        public int DCHvResistanceCurrent;
        public int DCMvResistanceCurrent;
        public int DCLvResistanceCurrent;
    ...
    public struct JobList
    {
        ...
        public bool CoreDCInsulation;
        public bool LossDcresistance;
        public bool Shortcircuitimpedance;

    }
    [JsonObject(MemberSerialization.OptOut)]
    public struct TcpJobTest
    {
        ...
        public string ResultText
        {
            get
            {
                ...
                if (failed)
                    ...
                    {
                        case MeasurementFunction.DCInsulation:
                            if (result.values[1].value == null) return this.Winding.ToString() + "绝缘电阻: " + result.values[1].ToString();
                            if (result.values[2].value == null) return this.Winding.ToString() + "绝缘电阻: " + result.values[1].ToString() + " 吸收比: " + result.values[2].ToString();
                        ...
                        case MeasurementFunction.Capacitance:
                            if (result.values[3] == null)
                                return this.Winding.ToString() + "电容量: " + result.values[2] + " 介质损耗：" + result.values[3].OriginText;
                            else if (result.values[3].PhysicalVariableType == Numerics.Quantities.QuantityName.Length)
                                return this.Winding.ToString() + "电容量: " + result.values[2] + " 介质损耗：" + SCEEC.Numerics.NumericsConverter.Value2Text((double)result.values[3].value / 1000, 4, -5, "", "", percentage: true, usePrefix: false).Trim();
                            else
                                return this.Winding.ToString() + "电容量: " + result.values[2] + " 介质损耗：" + SCEEC.Numerics.NumericsConverter.Value2Text((double)result.values[3].value, 4, -5, "", "", percentage: true, usePrefix: false).Trim();
                        case MeasurementFunction.DCResistance:
                            if (this.Terimal == null)
                            {
                                ...
                                rtn += "绕组\t电流\t电阻\n";//\t标号 
                                rtn += "AO\t";
                                rtn += result.values[1].OriginText;
                                rtn += "\t\t";
                                rtn += result.values[2].OriginText;
                                rtn += "\n";
                                rtn += "BO\t";
                                rtn += result.values[4].OriginText;
                                rtn += "\t\t";
                                rtn += result.values[5].OriginText;
                                rtn += "\n";
                                rtn += "CO\t";
                                rtn += result.values[7].OriginText;
                                rtn += "\t\t";
                                rtn += result.values[8].OriginText;
                                return rtn;
                            }
                            else
                            {
                                if (this.Terimal[0] == WindingTerimal.A)
                                    rtn = this.Winding.ToString() + this.Terimal[0].ToString() + this.Terimal[1].ToString() + "电流: " + result.values[1].OriginText + "\t直流电阻: " + result.values[2].OriginText;
                                else if (this.Terimal[0] == WindingTerimal.B)
                                    rtn = this.Winding.ToString() + this.Terimal[0].ToString() + this.Terimal[1].ToString() + "电流: " + result.values[4].OriginText + "\t直流电阻: " + result.values[5].OriginText;
                                else
                                    rtn = this.Winding.ToString() + this.Terimal[0].ToString() + this.Terimal[1].ToString() + "电流: " + result.values[7].OriginText + "\t直流电阻: " + result.values[8].OriginText;
                                return rtn;
                            }
                        case MeasurementFunction.OLTCSwitchingCharacter:
                        ...
                        case MeasurementFunction.BushingCapacitance:
                            if (result.values[3].value == null)
                                return this.Winding.ToString() + "套管" + this.Terimal[0].ToString() + "末屏的电容量: " + result.values[2] + " 介质损耗：" + result.values[3].OriginText;
                            else if (result.values[3].PhysicalVariableType == Numerics.Quantities.QuantityName.Length)
                                return this.Winding.ToString() + "套管" + this.Terimal[0].ToString() + "末屏的电容量: " + result.values[2] + " 介质损耗：" + SCEEC.Numerics.NumericsConverter.Value2Text((double)result.values[3].value / 1000, 4, -5, "", "", percentage: true, usePrefix: false).Trim();
                            else
