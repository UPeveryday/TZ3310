...
namespace SCEEC.MI.TZ3310
{
    ...
    public class UPanList
    {
        ...
        public string GetCurrent(Parameter.ZldzCurrent t)
        {
            if (Parameter.ZldzCurrent._10A == t)
            {
                return "02";
            }
            else if (Parameter.ZldzCurrent._3A == t)
                return "01";
            else
                return "00";
        ...
        private string CreateDcResistancePra(MeasurementItemStruct mi, JobList job)
        {
            if (mi.Terimal != null)
            {
                ...
                if (GetParameter.GetPraDCResistanceUpanCurrent(job) == Parameter.ZldzCurrent._1A) DcResistanceCurrent = "00";
                if (GetParameter.GetPraDCResistanceUpanCurrent(job) == Parameter.ZldzCurrent._3A) DcResistanceCurrent = "01";
                if (GetParameter.GetPraDCResistanceUpanCurrent(job) == Parameter.ZldzCurrent._10A) DcResistanceCurrent = "02";
                switch (Dcposition)
                {
            ...
            else
            {
                ...
                if (GetParameter.GetPraDCResistanceCurrentSpa(mi, job) == Parameter.ZldzCurrent._1A) DcResistanceCurrent = "00";
                if (GetParameter.GetPraDCResistanceCurrentSpa(mi, job) == Parameter.ZldzCurrent._3A) DcResistanceCurrent = "01";
                if (GetParameter.GetPraDCResistanceCurrentSpa(mi, job) == Parameter.ZldzCurrent._10A) DcResistanceCurrent = "02";
                rc.DcReNum++;
                rc.DcReNumEnable = true;
                return DcResistanceCurrent + Windingkind + WindingConfig;
            }
